    ...
    def __call__(self, M: torch.Tensor, chi: int) -> List[torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            if chi < D**2:
                ...
                idx = torch.argsort(evals, descending=True)[:chi]
                proj = evecs[:, idx]
                C = torch.diag(evals[idx])
                T = torch.einsum("ai,abcd,dj->ibcj", proj.conj(), T, proj)
            elif chi > D**2:
                pad_c = chi - C.shape[0]
                ...
                if self.cpu_offload:
                    with torch.autograd.graph.save_on_cpu(pin_memory=True):
                        T = torch.utils.checkpoint.checkpoint(
                            self._update_T, T, v, M, use_reentrant=False
                        )
                else:
                    T = torch.utils.checkpoint.checkpoint(
                        self._update_T, T, v, M, use_reentrant=False
